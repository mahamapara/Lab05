...
```{r}
model <- glm( judge_decision ~ judge + independent_decision + case_language + claim_location + logit_success, family = binomial, data = refugees)

summary(model)
```

$H_o$: claim_locationToronto = 0 
$H_a$: claim_locationToronoto != 0

$H_o$: claim_locationanother = 0 
$H_a$: claim_locationanother != 0

According to the hypothesis test the claim_location is not statistically significant to be involved in the model for predicting the judges' decision on granting a leave for appeal. So we fail to reject the null hypothesis for claim_locationToronto and claim_locationanother
...
```{r}
model1 <- glm( judge_decision ~ judge + independent_decision + case_language + logit_success, family = binomial, data = refugees)

summary(model1)
```

The reduced model suggests that the judges decision does not depend on the claim location, as we concluded based on our hypothsis test above. We can also say this based on our summary for model2 as the judges that were not statistically significant in model are still not significant in model1.
...
```{r}
model2 <- glm(judge_decision ~ independent_decision + case_language + logit_success, family = binomial, data = refugees)
...
Conclusion: Model 2 in the table above (which is our mod1- the model including the judge variable) is significantly better than model 1 and as model 2 (in the table which is our mod1) includes the judge variable, we would conclude that there is a statistically significant difference in the chances of granting an appeal for different judges.
...
e^1.51428 = approx. 3.67

If logit_success increases by one unit, while holding independent_decision and case_language fixed, the odds of the judge_decision to grant te leave for an appeal are 3.67 times as big.
...
If I was an immigrant I would not want to be heard by Judge Iacobucci since e^-2.56776 = approx. 0.077 which means that the odds of me having assiged a leave to appeal (while holding everyting else constant) increases very little compared to the other judges. 
